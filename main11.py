...
class BOT_Roleta:
  ...
  def __init__(self):
    ...
    self.token = '6934835748:AAEZmWW_tSd9TshAyuceofrXtu_jCa8VBCs'
    self.chat_id = '-1002110710539'
    self.url_API = 'https://api.scrapingcasinos.com/Evolution/Rodadas/XxxtremeLigh0001.txt'  # API - GET IN >> https://t.me/@guimatechofc
    ...
    self.link = "[JOGAR AGORA!](https://www.pingolbet.com/casino/lobby/live/game/3533)"
  ...
  def results(self):
    ...
    if self.win_results + self.empate_results + self.loss_results != 0:
      a = (100 / (self.win_results + self.empate_results + self.loss_results) *
...
""",
  ...
  def alert_gale(self):
    self.message_ids = self.bot.send_message(
        self.chat_id,
        text=f"""⚠️ Vamos para o {self.count}ª GALE""").message_id
    self.message_delete = True
    return
...
🤑  {self.link}
  ...
  def check_results(self, results):
    ...
    if self.tipo == "Colunas":
      if self.prediction == 23 and results in self.roulette_columns[
          "02"] or self.prediction == 23 and results in self.roulette_columns[
              "03"]:
        self.martingale("WIN", results)
        return

      elif self.prediction == 12 and results in self.roulette_columns[
          "01"] or self.prediction == 12 and results in self.roulette_columns[
              "02"]:
        self.martingale("WIN", results)
        return

      elif self.prediction == 13 and results in self.roulette_columns[
          "01"] or self.prediction == 13 and results in self.roulette_columns[
              "03"]:
        self.martingale("WIN", results)
        return
      ...
      if self.prediction == 23 and results in self.roulette_duzias[
          "02"] or self.prediction == 23 and results in self.roulette_duzias[
              "03"]:
        self.martingale("WIN", results)
        return

      elif self.prediction == 12 and results in self.roulette_duzias[
          "01"] or self.prediction == 12 and results in self.roulette_duzias[
              "02"]:
        self.martingale("WIN", results)
        return

      elif self.prediction == 13 and results in self.roulette_duzias[
          "01"] or self.prediction == 13 and results in self.roulette_duzias[
              "03"]:
        self.martingale("WIN", results)
        return
  ...
  def estrategy(self, results):
    print(datetime.datetime.now().strftime("%H:%M:%S"), self.game,
          results[0:5])
  ...
  def start(self):
    ...
    while True:
      try:
        ...
        results = [
            int(num.strip()) for num in results_str.split(',') if num.strip()
        ]
